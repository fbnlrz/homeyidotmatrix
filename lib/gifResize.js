...
// Pixels with alpha below this threshold get flattened onto the background.
// The display has no alpha channel, so anything semi-transparent has to make
// a binary decision; 128 (50%) matches PIL's default in the Python lib.
...
function _flattenTransparency(rgba, bgRgb) {
  for (let i = 0; i < rgba.length; i += 4) {
    if (rgba[i + 3] < ALPHA_TRANSPARENCY_THRESHOLD) {
