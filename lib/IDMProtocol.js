...
// Hard upper bound for PNG / GIF uploads. The protocol's chunk index is 16-bit
// LE; payloads anywhere near 64KB would silently truncate. 2MB is a generous
// cap that comfortably fits any 64×64 GIF the device can render.
const MAX_IMAGE_BYTES = 2_000_000;
// Hard upper bound on text input length. Beyond ~256 chars marquee playback
// becomes useless and the bitmap buffer grows into hundreds of KB.
...
function buildEffect(style, colors = [[255, 0, 0], [0, 0, 255]]) {
  if (!Array.isArray(colors)) {
    throw new TypeError('buildEffect: colors must be an array of [r,g,b] triples');
  }
  for (const c of colors) {
    if (!Array.isArray(c) || c.length < 3) {
      throw new TypeError('buildEffect: each color must be an [r,g,b] triple');
    }
  }
...
/**
 * Music-sync visualization — uses the device's built-in microphone to drive
 * a dancing figure animation. `type` selects the animation variant.
 * Source: python3-idotmatrix-library MusicSync.sendImageRythm.
 */
function buildMusicSyncStart(type = 1) {
  return Buffer.from([6, 0, 0, 2, type & 0xff, 1]);
}

function buildMusicSyncStop() {
  return Buffer.from([6, 0, 0, 2, 0, 0]);
}
...
function buildImagePayload(pngBuffer) {
  if (!Buffer.isBuffer(pngBuffer)) {
    throw new TypeError('buildImagePayload: pngBuffer must be a Buffer');
  }
  if (pngBuffer.length > MAX_IMAGE_BYTES) {
    throw new RangeError(`buildImagePayload: pngBuffer ${pngBuffer.length} > MAX_IMAGE_BYTES ${MAX_IMAGE_BYTES}`);
  }
  const chunks = _splitChunks(pngBuffer, 4096);
  // The `idk` header field is a 16-bit LE int — explicit mask so very large
  // PNGs don't silently end up with garbage high bytes (we already reject
  // them above, but keep the mask as belt-and-braces).
  const idk = (pngBuffer.length + chunks.length) & 0xffff;
...
function buildGifChunks(gifBuffer) {
  if (!Buffer.isBuffer(gifBuffer)) {
    throw new TypeError('buildGifChunks: gifBuffer must be a Buffer');
  }
  if (gifBuffer.length > MAX_IMAGE_BYTES) {
    throw new RangeError(`buildGifChunks: gifBuffer ${gifBuffer.length} > MAX_IMAGE_BYTES ${MAX_IMAGE_BYTES}`);
  }
      ...
      for (let gx = 0; gx < 8; gx++) {
        ...
        for (let dy = 0; dy < 2; dy++) {
          for (let dx = 0; dx < 2; dx++) {
            ...
            const y = gy * 2 + dy;
            if (x < 0 || x >= TEXT_BITMAP_WIDTH || y < 0 || y >= TEXT_BITMAP_HEIGHT) continue;
            const byteIdx = (y * (TEXT_BITMAP_WIDTH / 8)) + Math.floor(x / 8);
            if (byteIdx < 0 || byteIdx >= bitmap.length) continue;
...
function buildText(text, opts = {}) {
  if (typeof text === 'string' && text.length > MAX_TEXT_CHARS) {
    throw new RangeError(`buildText: text ${text.length} chars > MAX_TEXT_CHARS ${MAX_TEXT_CHARS}`);
  }
  const {
