...
function _renderTextBitmaps(text, font, { mirror = false } = {}) {
  const out = [];
  // To read correctly in a horizontal mirror (e.g. when the display sits
  // behind a camera that mirror-flips its preview), we both reverse the
  // character order AND horizontally flip each character's bitmap. The
  // mirror then flips both back, so the viewer sees the original text.
  const sequence = mirror ? Array.from(text).reverse() : Array.from(text || '');
