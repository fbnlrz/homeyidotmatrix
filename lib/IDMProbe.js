 ...
 */
async function testAeKey(client, keyInput, { onLog } = {}) {
  const log = onLog || (() => {});
  const crypto = require('crypto');

  // Accept ASCII string OR hex (32 chars = 16 B, 64 chars = 32 B). If the
  // input contains non-hex characters we assume it's the raw ASCII key.
  let keyBuf;
  const stripped = String(keyInput || '').trim();
  const looksHex = /^[0-9a-fA-F]+$/.test(stripped) && (stripped.length === 32 || stripped.length === 64);
  if (looksHex) {
  ...
  if (![16, 24, 32].includes(keyBuf.length)) {
    throw new Error(`AES key must be 16, 24 or 32 bytes; got ${keyBuf.length}`);
  }
  ...
  const probe = async (label, inputBytes) => {
    ...
    for (const { name, iv } of ivs) {
      try { candidates[`cbc-${name}`] = _cipherFirstBlock(`${aesCipher}-cbc`, keyBuf, iv, padded); }
      catch { /* skip */ }
    }
    if (keyBuf.length === 16) {
    ...
    candidates.hmacSha1   = crypto.createHmac('sha1',   keyBuf).update(input).digest().slice(0, 16).toString('hex');
  ...
  return {
    probedAt: new Date().toISOString(),
    keyAscii: keyBuf.length <= 64 && _isPrintable(keyBuf) ? keyBuf.toString('utf8') : null,
    ...
    interpretation: matchSet.size
      ...
      : ['No match across the tested modes. The IV scheme might be different (e.g. derived from BLE MAC, or per-message counter). Open AESUtils.encrypt() in jadx and look at exactly what bArr (the IV) is at the call site for ae01 writes.'],
  };
}
