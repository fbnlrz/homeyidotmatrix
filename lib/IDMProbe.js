 ...
 */
async function testAeKey(client, keyHex, { onLog } = {}) {
  const log = onLog || (() => {});
  const crypto = require('crypto');
  const keyBuf = Buffer.from(keyHex.replace(/[^0-9a-fA-F]/g, ''), 'hex');
  if (keyBuf.length !== 16) {
    throw new Error(`key must be 16 bytes (32 hex chars), got ${keyBuf.length}`);
  }
  ...
  const probe = async (label, inputBytes) => {
    ...
    // Device replies are `01 || tag` (17 bytes). Strip the prefix.
    const tags = responses.map(r => r.slice(2));

    const input = Buffer.from(inputBytes);
    const padded = _padPkcs7(input, 16);

    const ecb = _aesEcbFirstBlock(keyBuf, padded);
    const cbc = _aesCbcFirstBlock(keyBuf, Buffer.alloc(16), padded);
    let cmac;
    try { cmac = _aesCmac(keyBuf, input); } catch { cmac = null; }
    const hmac = crypto.createHmac('sha256', keyBuf).update(input).digest().slice(0, 16).toString('hex');

    const matches = [];
    for (const tag of tags) {
      if (tag === ecb)  matches.push({ mode: 'AES-ECB', tag });
      if (tag === cbc)  matches.push({ mode: 'AES-CBC zero-IV', tag });
      if (cmac && tag === cmac) matches.push({ mode: 'AES-CMAC', tag });
      if (tag === hmac) matches.push({ mode: 'HMAC-SHA256 trunc16', tag });
    }
    return {
      label,
      sentHex: input.toString('hex'),
      deviceResponses: responses,
      candidates: { ecb, cbc, cmac, hmac },
      matches,
    };
  };
  ...
  const anyMatch = samples.some(s => s.matches && s.matches.length);
  return {
    probedAt: new Date().toISOString(),
    keyHex: keyBuf.toString('hex'),
    samples,
    verdict: anyMatch ? 'KEY_MATCHES' : 'NO_MATCH',
    interpretation: anyMatch
      ? ['🎯 KEY ACCEPTED — found cipher mode matches the device. AE auth is cracked.']
      : ['No match across AES-ECB / AES-CBC / AES-CMAC / HMAC-SHA256. Either the key is wrong, the framing pads/prepends something we did not, or the device uses a different primitive (e.g. AES-CCM, CHACHA20-POLY1305).'],
  };
}
