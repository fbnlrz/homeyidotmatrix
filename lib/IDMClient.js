...
class IDMClient {
  ...
  /**
   * Subscribe to fa03 notifications. Homey BLE on different SDK versions
   * exposes this under different method names; try the known ones and
   * silently fall back to "no acks" if none match.
   */
  async subscribeNotifications(onData) {
    ...
    const candidates = ['subscribeToNotifications', 'subscribe', 'startNotifications', 'notify'];
    for (const m of candidates) {
      if (typeof c[m] === 'function') {
        try {
          const sub = await c[m](wrap);
          this.log && this.log(`[ble] subscribed via ${m}`);
          return sub || { unsubscribe: () => {} };
        } catch (e) {
          this.log && this.log(`[ble] ${m} failed:`, e.message);
        }
      }
    }
    this.log && this.log('[ble] No supported notification method on this Homey SDK; continuing without acks');
    return null;
  }
}
