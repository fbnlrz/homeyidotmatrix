  ...
  async write(payload, { withResponse = false, interChunkDelayMs = 0 } = {}) {
    ...
    const job = this._writeQueue.then(async () => {
      ...
      try {
        ...
        // A mid-transfer failure usually means the BLE link dropped.
        this._markDead(e.message);
        throw e;
      }
    });
    // Keep the stored chain alive even if this job rejects — otherwise one
    // failure would make every future write reject immediately.
