...
class IDMClient {
   ...
   */
  constructor({ homey, uuid, address, onLog, onConnected, onDisconnected, defaultWaitConnectedMs }) {
    this.homey = homey;
    this.uuid = uuid;
    ...
    // Device-configurable default for write() callers that don't pass an
    // explicit `waitConnectedMs`. The "fast_fail" device setting flips this
    // between 15s (patient) and 2s (loud).
    this._defaultWaitConnectedMs = Number.isFinite(defaultWaitConnectedMs)
      ? defaultWaitConnectedMs : WAIT_CONNECTED_TIMEOUT_MS;
  ...
  async write(payload, opts = {}) {
    const {
      ...
      ackPredicate = null,
      ackTimeoutMs = 1500,
      waitConnectedMs = this._defaultWaitConnectedMs,
    } = opts;
