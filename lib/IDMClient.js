...
class IDMClient {
  ...
  async _doConnect() {
    ...
    const service = _findService(services, 'fa00');
    if (!service) {
      await peripheral.disconnect().catch(() => {});
      const uuids = services.map(s => s.uuid).join(', ');
      throw new Error(`Service fa00 not found. Discovered: ${uuids || '(none)'}`);
    }
    const writeChar = _findChar(service, 'fa02');
    const notifyChar = _findChar(service, 'fa03');
    if (!writeChar) {
      await peripheral.disconnect().catch(() => {});
      throw new Error('Write characteristic fa02 not found on service fa00');
    }
